...
has $_ => ( is => 'rw' ) for qw( vertexBufferObject indexBufferObject vaoObject1 vaoObject2 );
...
sub InitializeVertexArrayObjects {
	my ( $self ) = @_;

	$self->vaoObject1( glGenVertexArrays_p( 1 ) );
	glBindVertexArray( $self->vaoObject1 );
	...
	$self->vaoObject2( glGenVertexArrays_p( 1 ) );
	glBindVertexArray( $self->vaoObject2 );

	my $posDataOffset = 4 * 3 * ( $self->numberOfVertices / 2 );
	$colorDataOffset += 4 * 4 * ( $self->numberOfVertices / 2 );

	#// Use the same buffer object previously bound to GL_ARRAY_BUFFER.
	glEnableVertexAttribArrayARB( 0 );
	glEnableVertexAttribArrayARB( 1 );
	glVertexAttribPointerARB_c( 0, 3, GL_FLOAT, GL_FALSE, 0, $posDataOffset );
	glVertexAttribPointerARB_c( 1, 4, GL_FLOAT, GL_FALSE, 0, $colorDataOffset );
	glBindBufferARB( GL_ELEMENT_ARRAY_BUFFER, $self->indexBufferObject );
